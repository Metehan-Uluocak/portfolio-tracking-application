...
const WATCHLIST_STORAGE_KEY = 'portfolio-watchlist-v1';
...
export async function saveWatchlistToStorage(items: WatchlistItem[]): Promise<void> {
  await AsyncStorage.setItem(WATCHLIST_STORAGE_KEY, JSON.stringify(items));
}

export async function loadWatchlistFromStorage(): Promise<WatchlistItem[]> {
